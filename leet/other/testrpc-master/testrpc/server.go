...
func (server *Server) Register(obj interface{}) error {
	...
	tp := reflect.TypeOf(obj)                          //拿到类型的指针
	val := reflect.ValueOf(obj)                        //拿到对象的指针
	serviceName := reflect.Indirect(val).Type().Name() //如果val指向的不是指针，拿到他的值
	log.Println(tp, val, serviceName)
...
func (server *Server) ServeConn(conn net.Conn) {
	...
	for {
		...
		// decode
		var req main.Request
		edcode, err := main.GetEdcode()
		if err != nil {
			return
		}
...
func (service *Service) RegisterGobArgsType() error {
	edcodeStr := new(main.Config).GetEdcodeConf()
	switch edcodeStr {
	case "gob":
